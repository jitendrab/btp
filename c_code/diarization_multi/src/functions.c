...
/******************************************************************************
this function calculates delta BIC by using formulla of involving covariance matrices 
both states are modelled using signle gaussians And combined states are also modelled 
as single gaussian
******************************************************************************/
void CalculateBIC( float **deltaBIC,   VECTOR_OF_F_VECTORS *features,    VECTOR_OF_F_VECTORS *allMixtureMeans, 
		   ...
		   int *numMixEachState,               int *numElemEachState)
{
  int                               s = 0, i = 0, j = 0, k = 0, count = 0, d= 0;
  float                             L0 = 0, L1 = 0;
  float                             *det;
  ...
  /* ASSUME EACH STATE IS MODELLED BY A MULTI COMPONENT GAUSSIAN MIXTURE MODEL*/
  for(i = 0; i < *numStates; i++){
    count = 0;
    for(k = 0; k < totalNumFeatures; k++){
      if( stateSeq[k] == i  ){
    ...
    for(j = 0; j < DIM; j++){
      if(mixtureVars[0]->array[j] > 0.0001)
	det[i] = det[i] + log(mixtureVars[0]->array[j]);
    }
  }
  for(i = 0; i < *numStates; i++)
    printf("det[%d]: %f\n", i, det[i]);
  ...
  for(i = 0; i < *numStates; i++){
	...
	detSigma = detSigma + log(mixtureVars[0]->array[k]);
      ...
      float Penalty = 0.5 * (DIM + 0.5 * DIM * (DIM + 1)) * log(nI + nJ);
      deltaBIC[i][j] = (nI + nJ) * detSigma - nI * det[i] - nJ * det[j] - LAMBDA * Penalty;
      printf("i:%d  j:%d  deltaBIC: %f P: %f\n", i, j , deltaBIC[i][j], LAMBDA*Penalty);
    }
  }
}
