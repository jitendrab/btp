 ...
 *  Last modified:  Sat 19-Mar-2011 09:36:14 by hema
 *  $Id: DspLibrary.c,v 1.3 2007/06/14 08:22:59 hema Exp hema $
 *
 *  Bugs:	
 *
...
float *AutoCorr (float *signal, int npts) {
  ...
  for (i = 1; i <= npts; i++){
    ...
    lim = npts-i;
    for (j = 1; j <= lim ; j++)
      autoCorr[i] = autoCorr[i]+signal[j]*signal[j+i];
  }
  return(autoCorr);
}
...
void MASignal(float *residual,float *MASignal,int numSamples,
	  ...
	  for (i = 0; i < numSamples; i++){
	    MASignal[i] = coefZero[0]*residual[i+orderZ]; 
	    sum1 = 0.0;
	    if (orderZ != 0)
	      for (j = 1; j < orderZ; j++)
	      sum1 = sum1 +coefZero[j]*residual[i+orderZ-j];
	    sum2 = 0.0;
	    MASignal[i] = MASignal[i] + sum1 - sum2;
...
void LinearTransformationOfFVector (F_VECTOR *inVect, VECTOR_OF_F_VECTORS 
  ...
  for (i = 0; i < numRows; i++){
    outVect->array[i] = 0.;
    for (j = 0; j < numCols; j++)
      outVect->array[i] += inVect->array[j]*melCepstrumCosineTransform[i]->array[j];
  }
}
...
float ComputeSpectralFlatness(float *array, float *residual, int order, int frameSize, int frameShift) { 
  ...
  sigEnergy = ComputeEnergy(&array[1], frameSize);
  coef = (float *) AllocFloatArray(coef, order+1);
  LpAnal(array, residual, frameSize, frameShift, coef, order, &gain);
  resEnergy = ComputeEnergy(&residual[1], frameSize);
  free (coef);
...
int GradientFrames(float *frame1, float *frame2, int numPts, int frameAdvanceSamples, 
  ...
  fEnergy1 = (float) ComputeEnergy(frame1, numPts);
  fEnergy2 = (float) ComputeEnergy(frame2, numPts);
  ...
  resEnergy1 = ComputeEnergy(residual1, numPts);
  LpAnal(&frame2[-1], &residual2[-1],numPts, frameAdvanceSamples, coef, 10, &gain);
  resEnergy2 = ComputeEnergy(residual2, numPts);
...
void VoicedUnvoiced(short *waveform, long samples, short *vU, 
		    int frameShift, int frameSize, int order, float thresZero, float thresEnergy, float thresSpecFlatness, int percentFrames) {
  ...
  energy[i] = ComputeEnergy(&array[1],frameSize);
  ...
  /*  printf("zero %f energy %f flat %f \n", zeroCrossings[i], energy[i], specFlatness[i]);
      scanf("*c"); */
 if (percentFrames == 1) {
   QuickSort(energy, 0, numFrames-1);
   frameNumber = (int) (thresEnergy*(numFrames-1));
   energyThres = energy[frameNumber];
   QuickSort(zeroCrossings, 0, numFrames-1);
   frameNumber = (int) ((1-thresZero)*(numFrames-1));
   zeroThres = zeroCrossings[frameNumber];
   QuickSort(specFlatness, 0, numFrames-1);
   frameNumber = (int) ((1-thresSpecFlatness)*(numFrames-1));
   specThres = specFlatness[frameNumber];
 } else {
...
/*-------------------------------------------------------------------------
 *  ComputeThresholdEnergy -- Computes the threshold on energy for a waveform
 *    Args: asdf, threshold, percentFrames	
 *    Returns:	float
 *    Throws:	
 *    See:	
 *    Bugs:	
...
/*float ComputeThresholdEnergy(ASDF *asdf, float threshold, int percentFrames)
{
  ...
  float energyThres;
  if (percentFrames == 1) {
    energyFrames = (float *) malloc(asdf->numFrames*sizeof(float));
    for (i = 0; i < asdf->numFrames; i++) 
      energyFrames[i] = ((F_VECTOR *) GsfRead(asdf, i, "frameEnergy"))->array[0];
    QuickSort(energyFrames, 0, asdf->numFrames-1);
    frameNumber = (int) (threshold*asdf->numFrames);
    energyThres = energyFrames[frameNumber];
    free(energyFrames);
  } else {
    for (i = 0; i < asdf->numFrames; i++) 
      ave = ave + ((F_VECTOR *) GsfRead(asdf, i, "frameEnergy"))->array[0];               ;
    ave = ave/asdf->numFrames;
    energyThres = ave*threshold;
  }
  return (energyThres);
}	/*  End of ComputeThresholdEnergy		End of ComputeThresholdEnergy   */


/*-------------------------------------------------------------------------
 *  ComputeThresholdZeroCrossing -- Computes the threshold on Zero crossings for a waveform
 *    Args: asdf, threshold, percentFrames	
 *    Returns:	float
 *    Throws:	
 *    See:	
...
/*float ComputeThresholdZeroCrossing(ASDF *asdf, float threshold, int percentFrames)
{
  int i, frameNumber;
  float ave = 0;
  float *zeroCrossingFrames;
  float zeroCrossingThres;
  if (percentFrames == 1) {
    zeroCrossingFrames = (float *) malloc(asdf->numFrames*sizeof(float));
    for (i = 0; i < asdf->numFrames; i++) 
      zeroCrossingFrames[i] = ((F_VECTOR *) FrameComputeZerocrossing(asdf, i, f->array[0];
    QuickSort(zeroCrossingFrames, 0, asdf->numFrames-1);
    frameNumber = (int) ((1-threshold)*asdf->numFrames);
    zeroCrossingThres = zeroCrossingFrames[frameNumber];
    free(zeroCrossingFrames);
  } else {
    for (i = 0; i < asdf->numFrames; i++) 
      ave = ave + ((F_VECTOR *) GsfRead(asdf, i, "frameZeroCrossing"))->array[0];               ;
    ave = ave/asdf->numFrames;
    zeroCrossingThres = ave*threshold;
  }
  return (zeroCrossingThres);
}	/*  End of ComputeThresholdZeroCrossing		End of ComputeThresholdZeroCrossing   */


/*-------------------------------------------------------------------------
 *  ComputeThresholdSpectralFlatness -- Computes the threshold on SpectralFlatness for a waveform
 *    Args: asdf, threshold, percentFrames	
 *    Returns:	float
 *    Throws:	
 *    See:	
      ...
      /*float ComputeThresholdSpectralFlatness(ASDF *asdf, float threshold, int percentFrames)
{
  int i, frameNumber;
  float ave = 0;
  float *spectralFlatnessFrames;
  float spectralFlatnessThres;
  if (percentFrames == 1) {
    spectralFlatnessFrames = (float *) malloc(asdf->numFrames*sizeof(float));
    for (i = 0; i < asdf->numFrames; i++) 
      spectralFlatnessFrames[i] = ((F_VECTOR *) GsfRead(asdf, i, "frameSpectralFlatness"))->array[0];
    QuickSort(spectralFlatnessFrames, 0, asdf->numFrames-1);
    frameNumber = (int) ((1-threshold)*asdf->numFrames);
    spectralFlatnessThres = spectralFlatnessFrames[frameNumber];
    free(spectralFlatnessFrames);
  } else {
    for (i = 0; i < asdf->numFrames; i++) 
      ave = ave + ((F_VECTOR *) GsfRead(asdf, i, "frameSpectralFlatness"))->array[0];               ;
    ave = ave/asdf->numFrames;
    spectralFlatnessThres = ave*threshold;
  }
  return (spectralFlatnessThres);
...
float*
MinGdCepstrum_DCT(float *signal,     
                  int npts, int nfft, int mfft,      
			      int gdSmthWinSize, int numCepstrum, 
                  float *minGdCepstrum, 
                  float gamma, 
                  int startIndex, int endIndex)
{
  ...
  for (i = 0; i < numCepstrum; i++) 
    minGdCepstrum[i] = minGdFvect->array[i];
  return(minGdCepstrum);  
}
...
float *ModGdLogSmthCepstrumNcN(float *signal,int npts,int nfft,int mfft, 
  ...
  /* Reduce dynamic range of modGd if necessary */
    ...
    if (medOrder != 0) {
      MedianSmoothArray(modGd, nfBy2+1, medOrder, modGdSmth);
      for (i = 2; i <= nfBy2; i++)
...
float *StandardModGdLP(float *signal,int npts,int nfft,int mfft, int lpOrder,
  ...
  if (flag == 0) {
    ...
    coef = (float *) calloc(lpOrder+1, sizeof(float));
    phase = (float *) calloc(nfft+1, sizeof(float));
    modGd = (float *) calloc(nfft/2, sizeof(float));
    cSig = (complex *) calloc (nfft+1, sizeof(complex));
    cfSig = (complex *) calloc (nfft+1, sizeof(complex));
    cfTemp1 = (complex *) calloc (nfft+1, sizeof(complex));
    cfTemp2 = (complex *) calloc (nfft+1, sizeof(complex));
 ...
 LpAnal(sigTemp, resEnergy, npts, 0, coef, lpOrder, &gain);
  ...
  if (tempVal != 0.0) {
    ...
    cfTemp2[1].re = 0;					    
    modGd[0] = cfTemp2[1].re;
    if (medOrder != 0) 
      MedianSmoothArray(&modGd[0], nfBy2, medOrder, modGdSmth);
    else
      for (i = 0; i < nfBy2; i++)
        modGdSmth[i] = modGd[i];
    return(modGdSmth);
