...
int main(int argc, char *argv[]){
  if(argc < 2){
    fprintf(stderr, "Usage: <binary> <path to feature file(HTK format)>\n");
    exit(0);
  }
  ...
  TotalFeatures = count-1;
  allFeaturesCount = i-1;
			  ...
			  float **posterior,	  int *numElemEachState,                 float *Pi)
{
  ...
  writePlotFile(numFeatures, numStates, stateSeq);
  //WRITE RTTM FILE
...
/******************************************************************************/
void writePlotFile(int totalNumFeatures, int *numStates, int *featureState){
  int                    i = 0, j = 0, s= 0, d = 0;
  FILE                   *plotFile;  
